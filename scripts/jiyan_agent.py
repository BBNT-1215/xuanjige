...
启动：
  python3 scripts/jiyan_agent.py [--poll-interval 5]
...
def emit_event(event_type, payload):
    ...
    print(f"[机研] 📡 事件已记录: {event_type}", flush=True)
...
def on_task_done(task_id):
    ...
    if not evolution_engine:
        print(f"[机研] ⚠️ 进化引擎未加载，跳过任务 {task_id}")
        return
    
    try:
        print(f"[机研] 🔄 触发进化分析: {task_id}", flush=True)
        result = evolution_engine.evolve_task(task_id)
        
        if 'error' in result:
            print(f"[机研] ⚠️ {result['error']}")
        else:
            print(f"[机研] ✅ 进化完成: {result.get('task_id')} - 进化项: {result.get('evolved_items', [])}")
    except Exception as e:
        print(f"[机研] ❌ 进化分析失败: {e}", flush=True)


def on_task_created(task_id, task_title, official):
    """新任务创建时记录"""
    print(f"[机研] 📝 新任务: [{task_id}] {task_title} (旨意: {official})", flush=True)
...
def daily_report():
    ...
    try:
        ...
        print(f"\n{'='*50}")
        print(f"[机研] 📊 每日进化报告 ({report.get('timestamp', '')})")
        print(f"{'='*50}")
        ...
        print(f"[机研] ⚠️ 报告生成失败: {e}", flush=True)
...
""", flush=True)
    ...
    if role_lib:
        try:
            role_lib.scan_roles()
            print(f"[机研] ✅ Role库扫描完成", flush=True)
        except Exception as e:
            print(f"[机研] ⚠️ Role库扫描失败: {e}", flush=True)
    
    # 扫描已完成任务并进化（启动时一次性执行）
    print(f"[机研] 🔍 启动时扫描已完成任务...", flush=True)
    tasks = _read_json(TASKS_FILE) or []
    done_tasks = [t for t in tasks if t.get('state') == 'Done']
    print(f"[机研] 发现 {len(done_tasks)} 个已完成任务", flush=True)
    ...
    print(f"[机研] ▶ 常驻监听中（轮询间隔 {args.poll_interval}秒）...", flush=True)
    ...
    while True:
        try:
            ...
            print(f"\n[机研] 🛑 收到中断信号，正在停止...", flush=True)
            break
        except Exception as e:
            print(f"[机研] ⚠️ 异常: {e}", flush=True)
            time.sleep(args.poll_interval)
