#!/usr/bin/env python3
"""
Hermestrix Engine - 吏部常驻进程 (LibuAgent)
...
用法：
  python3 engine/libu_agent.py [--once] [--interval <seconds>]
  --once    : 运行一次处理后退出（用于测试）
  --verbose : 输出详细日志
"""
...
AGENT_PID_FILE = DATA_DIR / "libu_agent.pid"
...
def _atomic_json_update(path: pathlib.Path, modifier, default=None):
    for attempt in range(3):
        try:
            ...
            if attempt == 2:
                print(f"[libu_agent] Failed to update {path}: {e}")
            time.sleep(0.1)
...
class LibuAgent:
                    ...
                    for evt in new_events:
                        ...
                        for h in handlers:
                            try:
                                ...
                                print(f"[libu_agent] Handler error for {evt_type}: {e}")
                        self._stats["events_processed"] += 1
            except Exception as e:
                print(f"[libu_agent] Event loop error: {e}")
    ...
    def _health_loop(self):
        ...
        while self.running and not self._shutdown.is_set():
            ...
            try:
                ...
                print(f"[libu_agent] Health check error: {e}")
    ...
    def _decay_loop(self):
        ...
        while self.running and not self._shutdown.is_set():
            ...
            try:
                ...
                print(f"[libu_agent] Decay error: {e}")
    ...
    def run(self, once: bool = False):
        ...
        print(f"[libu_agent] 吏部常驻进程启动 (PID={os.getpid()})")
        print(f"[libu_agent] 健康检查: {self.DEFAULT_HEALTH_INTERVAL}s")
        print(f"[libu_agent] 衰减管理: {self.DEFAULT_DECAY_INTERVAL}s")
        print(f"[libu_agent] 事件轮询: {self.DEFAULT_POLL_INTERVAL}s")
    ...
    def stop(self):
        """停止进程"""
        print("[libu_agent] 停止中...")
        self.running = False
        self._shutdown.set()
        self._cleanup_pid()

    def _on_signal(self, signum, frame):
        print(f"[libu_agent] 收到信号 {signum}")
        self.stop()
...
if __name__ == "__main__":
    ...
    if args.interval:
        LibuAgent.DEFAULT_HEALTH_INTERVAL = args.interval
